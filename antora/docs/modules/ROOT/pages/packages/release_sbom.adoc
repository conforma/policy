...
* To exclude this rule, add `sbom.disallowed_packages_provided` to the `exclude` list in your policy config. You can also exclude the entire package with `sbom`, or exclude by collection with `@collection-name`.
...
* Code: `sbom.found`
* To exclude this rule, add `sbom.found` to the `exclude` list in your policy config. You can also exclude the entire package with `sbom`, or exclude by collection with `@collection-name`.
