...
* To exclude this rule, add `rpm_pipeline.invalid_pipeline` to the `exclude` list in your policy config. You can also exclude the entire package with `rpm_pipeline`, or exclude by collection with `@collection-name`.
