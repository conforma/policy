...
* To exclude this rule, add `rpm_ostree_task.builder_image_param` to the `exclude` list in your policy config. You can also exclude the entire package with `rpm_ostree_task`, or exclude by collection with `@collection-name`.
...
* To exclude this rule, add `rpm_ostree_task.rule_data` to the `exclude` list in your policy config. You can also exclude the entire package with `rpm_ostree_task`, or exclude by collection with `@collection-name`.
