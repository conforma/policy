...
* To exclude this rule, add `git_branch.git_branch` to the `exclude` list in your policy config. You can also exclude the entire package with `git_branch`, or exclude by collection with `@collection-name`.
