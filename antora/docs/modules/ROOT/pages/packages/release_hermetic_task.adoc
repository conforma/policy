...
* To exclude this rule, add `hermetic_task.hermetic` to the `exclude` list in your policy config. You can also exclude the entire package with `hermetic_task`, or exclude by collection with `@collection-name`.
