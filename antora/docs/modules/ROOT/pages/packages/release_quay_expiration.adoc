...
* To exclude this rule, add `quay_expiration.expires_label` to the `exclude` list in your policy config. You can also exclude the entire package with `quay_expiration`, or exclude by collection with `@collection-name`.
