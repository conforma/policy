...
* To exclude this rule, add `annotations.expires_on_format` to the `exclude` list in your policy config. You can also exclude the entire package with `annotations`, or exclude by collection with `@collection-name`.
