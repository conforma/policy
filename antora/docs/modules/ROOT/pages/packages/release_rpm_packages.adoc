...
* To exclude this rule, add `rpm_packages.unique_version` to the `exclude` list in your policy config. You can also exclude the entire package with `rpm_packages`, or exclude by collection with `@collection-name`.
