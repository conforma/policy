...
* To exclude this rule, add `step_images.step_images_accessible` to the `exclude` list in your policy config. You can also exclude the entire package with `step_images`, or exclude by collection with `@collection-name`.
