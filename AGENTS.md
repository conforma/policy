...
Policies access the image under evaluation via `input.image`. The canonical reference for its shape
is the acceptance sample at `acceptance/samples/policy-input-golden-container.json`, generated by
`ec validate image --output policy-input`.

**Available fields:**

| Field | Type | Description |
|-------|------|-------------|
| `input.image.config` | object | OCI image config (`.Labels`, `.Env`, `.Cmd`, `.WorkingDir`) |
| `input.image.files` | object | Map of in-image file paths to their parsed content (used by OLM/SBOM rules) |
| `input.image.parent` | object | Parent image metadata (`.ref`, `.config`) |
| `input.image.ref` | string | Fully-qualified image reference including digest (e.g., `registry/repo@sha256:...`) |
| `input.image.signatures` | list | Cosign signature envelopes attached to the image |
| `input.image.source` | object | Source SCM metadata (`.git.url`, `.git.revision`) |

**Image digest access pattern:** To obtain the image digest, use the `image.parse` helper from
`policy/lib/image/`:
...
contract. Tests that mock `input.image.digest` conceal this gap because OPA allows `with` to bind
any path regardless of whether the field exists at runtime.
