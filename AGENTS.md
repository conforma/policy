...
## Input Contract (`input.image`)

Policies access the image under evaluation via `input.image`. The canonical reference for its shape is the acceptance sample at `acceptance/samples/policy-input-golden-container.json`, generated by `ec validate image --output policy-input`.

**Available fields:**

- **`input.image.config`:** OCI image config (`.Labels`, `.Env`, `.Cmd`, `.WorkingDir`)
- **`input.image.files`:** Map of in-image file paths to their parsed content — only populated for operator bundle images, absent from the golden-container sample
- **`input.image.parent`:** Parent image metadata (`.ref`, `.config`)
- **`input.image.ref`:** Fully-qualified image reference including digest (e.g., `registry/repo@sha256:...`)
- **`input.image.signatures`:** Cosign signature envelopes attached to the image
- **`input.image.source`:** Source SCM metadata (`.git.url`, `.git.revision`)

**Image digest access pattern:** To obtain the image digest, use the `image.parse` helper from `policy/lib/image/`:

```rego
import data.lib.image

ref := image.parse(input.image.ref)   # returns {digest, repo, tag}
ref.digest                             # "sha256:..."
```

Do **not** access `input.image.digest` directly — this field is not part of the established input contract. The existing usage in `policy/release/lib/attestations.rego` (`_statement_subject_matches_image`) is a known exception pending migration to `image.parse`. Tests that mock `input.image.digest` conceal this gap because OPA allows `with` to bind any path regardless of whether the field exists at runtime.
