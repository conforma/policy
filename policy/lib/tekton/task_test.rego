...
test_tasks_from_attestation if {
	...
	utils.assert_equal(expected, tekton.tasks(attestation))
}
...
test_tasks_from_slsav1_tekton_attestation if {
	...
	utils.assert_equal({expected_task}, tasks)
}
...
test_tasks_from_pipeline if {
	...
	utils.assert_equal(expected, tekton.tasks(pipeline))
}

test_tasks_from_partial_pipeline if {
	utils.assert_empty(tekton.tasks({"kind": "Pipeline"}))
	utils.assert_empty(tekton.tasks({"kind": "Pipeline", "spec": {}}))

	git_clone := {"taskRef": {"name": "git-clone"}}
	expected := {{"taskRef": {"name": "git-clone"}}}
	utils.assert_equal(expected, tekton.tasks({"kind": "Pipeline", "spec": {"tasks": [git_clone]}}))
	utils.assert_equal(expected, tekton.tasks({"kind": "Pipeline", "spec": {"finally": [git_clone]}}))
}

test_tasks_not_found if {
	utils.assert_empty(tekton.tasks({}))
}

test_task_param if {
	task := {"params": [{"name": "NETWORK", "value": "none"}]}
	utils.assert_equal("none", tekton.task_param(task, "NETWORK"))
	not tekton.task_param(task, "missing")
}

test_task_result if {
	task := {"results": [{"name": "SPAM", "value": "maps"}]}
	utils.assert_equal("maps", tekton.task_result(task, "SPAM"))
	not tekton.task_result(task, "missing")

	slsav1_task := resolved_slsav1_task("task-name", [], [{"name": "SPAM", "value": "maps"}])
	utils.assert_equal("maps", tekton.task_result(slsav1_task, "SPAM"))
...
test_tasks_from_attestation_with_spam if {
	...
	utils.assert_equal(expected_tasks, tekton.tasks(attestation))
	expected_names := {"git-clone", "buildah", "buildah[HERMETIC=true]", "weird", "weird[SPAM=MAPS]", "summary"}
	utils.assert_equal(expected_names, tekton.tasks_names(attestation))
}
...
test_tasks_from_pipeline_with_spam if {
	...
	utils.assert_equal(expected_tasks, tekton.tasks(pipeline))

	expected_names := {"git-clone", "buildah", "buildah[NETWORK=none]", "weird", "weird[SPAM=MAPS]", "summary"}
	utils.assert_equal(expected_names, tekton.tasks_names(pipeline))
}

test_build_task if {
	expected := [_good_build_task, _good_source_build_task]
	utils.assert_equal(expected, tekton.build_tasks(_good_attestation))
}
...
test_pre_build_tasks if {
	...
	utils.assert_equal(expected, tekton.pre_build_tasks(_good_attestation))
}
...
test_git_clone_task if {
	...
	utils.assert_equal([expected], tekton.git_clone_tasks(_good_attestation))
}
...
test_source_build_task if {
	...
	utils.assert_equal([expected], tekton.source_build_tasks(_good_attestation))
}
...
test_task_names_local if {
	...
	utils.assert_equal(expected, tekton.task_names(task))
}

test_task_data_no_bundle_ref if {
	utils.assert_equal({"name": "name"}, tekton.task_data({"ref": {"name": "name"}}))
}

test_missing_required_tasks_data if {
	utils.assert_equal(tekton.missing_required_tasks_data, true) with data["required-tasks"] as []
	utils.assert_equal(tekton.missing_required_tasks_data, false) with data["required-tasks"] as _time_based_required_tasks
}
...
test_pipeline_task_slsav1 if {
	...
	utils.assert_equal(tekton.pipeline_task_name(slsav1_task_spec), "build-push")
	utils.assert_equal(tekton.pipeline_task_name(slsav1_task("my-pipeline")), "my-pipeline")
}
...
test_pipeline_task_slsav02 if {
	...
	utils.assert_equal(tekton.pipeline_task_name(slsav02_inline_task_spec), "copy-settings")

	task := {"name": "git-clone-p", "ref": {"name": "git-clone"}}
	utils.assert_equal(tekton.pipeline_task_name(task), "git-clone-p")
}
...
test_taskrun_labels_slsa02 if {
	...
	utils.assert_equal(tekton.task_labels(task), {"l1": "v1", "l2": "v2"})
}
...
test_taskrun_annotations_slsa02 if {
	...
	utils.assert_equal(tekton.task_annotations(task), {"a1": "v1", "a2": "v2"})
}
...
test_task_result_endswith if {
	...
	utils.assert_equal(["1234-image1", "image1"], tekton.task_result_endswith(task1, "ARTIFACT_URI"))
}
