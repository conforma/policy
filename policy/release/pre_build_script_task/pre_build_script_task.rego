...
deny contains result if {
	...
	result := utils.result_helper_with_term(rego.metadata.chain(), [image_ref], repo)
}
...
deny contains result if {
	...
	result := utils.result_helper(rego.metadata.chain(), [tekton.task_name(task)])
}
...
deny contains result if {
	...
	not image.parse(ref)
	result := utils.result_helper(rego.metadata.chain(), [ref])
}
...
deny contains result if {
	...
	result := utils.result_helper(rego.metadata.chain(), [pre_build_image])
}

_pre_build_script_runner_image_param := "SCRIPT_RUNNER_IMAGE"

_image_ref_permitted(image_ref) if {
	allowed_prefixes := utils.rule_data(_rule_data_allowed_registries_key)
