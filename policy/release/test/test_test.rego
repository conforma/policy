...
test_needs_non_empty_data if {
	...
	utils.assert_equal_results(test.deny, {{
...
test_success_data if {
	...
	utils.assert_empty(test.deny) with input.attestations as attestations
}
...
test_failure_data if {
	...
	utils.assert_empty(test.warn) with input.attestations as attestations
	utils.assert_equal_results(test.deny, {
		{
	...
	utils.assert_equal_results(test.warn, {
		{
...
test_missing_wrong_attestation_type if {
	...
	utils.assert_empty(test.deny) with input.attestations as [tr, tr_slsav1]
}
...
test_rule_data_provided if {
	...
	utils.assert_equal_results(test.deny, expected) with data.rule_data as d
}
