#
# METADATA
# title: Trusted Task checks (Simplified Format)
# description: >-
#   This package implements trusted task verification using the simplified rule-based format.
#   It supports both legacy trusted_tasks data and new trusted_task_rules format with
#   unified allowed_tasks and denied_tasks sections.
#
...
is_trusted_task(task) if {
	...
	_is_explicitly_allowed(ref.key)
	not _is_explicitly_denied(ref.key)
}

# Check if task is explicitly allowed by rules
_is_explicitly_allowed(ref_key) := true if {
	some allowed_task in _trusted_task_rules_data.allowed_tasks
	glob.match(allowed_task.task, [], ref_key)
	_allowance_effective(allowed_task)
	_version_constraints_met(allowed_task, ref_key)
}

# Check if task is explicitly denied by rules
_is_explicitly_denied(ref_key) := true if {
	some denied_task in _trusted_task_rules_data.denied_tasks
	glob.match(denied_task.task, [], ref_key)
	_denial_effective(denied_task)
	_version_constraints_met(denied_task, ref_key)
}

# Check if allowance is effective (past effective_on date)
_allowance_effective(allowance) := true if {
...
# Check if denial is effective (past effective_on date)
_denial_effective(denial) := true if {
...
_version_constraints_met(rule, ref_key) := true if {
	not _contains_version_constraint(rule.task)
} else := true if {
	version := _extract_version_from_ref(ref_key)
	_semver_satisfies_constraint(version, rule.task)
}

# Check if task pattern contains version constraints
_contains_version_constraint(task_pattern) := true if {
	regex.match(`:[0-9]`, task_pattern)
}

# Extract version from reference
_extract_version_from_ref(ref_key) := version if {
...
_semver_satisfies_constraint(version, constraint) := true if {
	# Extract version constraint from task pattern
	version_part := _extract_version_from_task_pattern(constraint)
	_semver_satisfies(version, version_part)
}

# Extract version constraint from task pattern
_extract_version_from_task_pattern(task_pattern) := version_constraint if {
	parts := split(task_pattern, ":")
	version_constraint := parts[count(parts) - 1]
}

# Semantic version constraint checking
_semver_satisfies(version, constraint) := true if {
	# Simple implementation - would need full semver logic
	version == constraint
} else := true if {
	startswith(constraint, ">=")
	required_version := substring(constraint, 2, -1)
	_semver_compare(version, required_version) >= 0
} else := true if {
	startswith(constraint, "<")
	required_version := substring(constraint, 1, -1)
	_semver_compare(version, required_version) < 0
}

# Simple semantic version comparison
_semver_compare(version1, version2) := result if {
	# Simplified semver comparison - would need full implementation
	version1 == version2
	result := 0
} else := 1 if {
	# Placeholder logic
...
# Data validation for trusted_task_rules format (simplified)
_data_errors contains error if {
	some e in j.validate_schema(
