...
test_allow_when_permitted if {
	utils.assert_empty(attestation_type.deny) with input.attestations as mock_data(good_type)
	utils.assert_empty(attestation_type.deny) with input.attestations as mock_data(good_type_v1)
}

test_deny_when_not_permitted if {
	expected_msg := sprintf("Unknown attestation type '%s'", [bad_type])
	utils.assert_equal_results(attestation_type.deny, {{
...
test_deny_when_pipelinerun_attestation_founds if {
	...
	utils.assert_equal_results(attestation_type.deny, expected) with input.attestations as attestations
}
...
test_rule_data_validation if {
	...
	utils.assert_equal_results(attestation_type.deny, expected) with data.rule_data as d
