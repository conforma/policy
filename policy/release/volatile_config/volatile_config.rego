...
warn contains result if {
	...
	vc.warning_category(rule.config) == "pending"
...
warn contains result if {
	some rule in _applicable_rules
	vc.warning_category(rule.config) == "expiring"
	days := vc.days_until_expiration(rule.config)
	result := lib.result_helper(
		rego.metadata.chain(),
...
warn contains result if {
	some rule in _applicable_rules
	vc.warning_category(rule.config) == "no_expiration"
	result := lib.result_helper(
		rego.metadata.chain(),
...
warn contains result if {
	some rule in _applicable_rules
	vc.warning_category(rule.config) == "expired"
	result := lib.result_helper(
		rego.metadata.chain(),
...
warn contains result if {
	some rule in _applicable_rules
	vc.warning_category(rule.config) == "invalid"
	result := lib.result_helper(
		rego.metadata.chain(),
		[
			rule.type,
...
_applicable_rules contains rule if {
	...
	vc.is_rule_applicable(config, _image_ref, _image_digest, _component_name)
...
_component_name := input.component_name if {
