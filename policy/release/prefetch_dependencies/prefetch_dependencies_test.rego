...
test_mode_permissive_violation if {
	utils.assert_equal_results(prefetch_dependencies.deny, {{
...
test_mode_not_permissive_pass if {
	utils.assert_empty(prefetch_dependencies.deny) with input as _attestation("prefetch-dependencies", "strict")
}

test_missing_mode_param_pass if {
	utils.assert_empty(prefetch_dependencies.deny) with input as _attestation_without_mode("prefetch-dependencies")
}

test_task_not_present_pass if {
	utils.assert_empty(prefetch_dependencies.deny) with input as _attestation("some-other-task", "permissive")
}
...
test_oci_ta_mode_not_permissive_pass if {
	utils.assert_empty(prefetch_dependencies.deny) with input as _attestation("prefetch-dependencies-oci-ta", "strict")
}
