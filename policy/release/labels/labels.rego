...
warn contains result if {
	...
	result := _with_effective_on(
		utils.result_helper_with_term(rego.metadata.chain(), [name, description], name),
		optional_label,
	)
}
...
deny contains result if {
	...
	is_null(manifest)
	result := utils.result_helper(rego.metadata.chain(), [input.image.ref])
}
...
deny contains result if {
	...
	some deprecated_label in utils.rule_data("deprecated_labels")
	label.name == deprecated_label.name
	result := _with_effective_on(
		utils.result_helper_with_term(
			rego.metadata.chain(),
...
deny contains result if {
	...
	result := object.union(utils.result_helper(rego.metadata.chain(), []), err)
}
...
deny contains result if {
	...
	result := _with_effective_on(
		utils.result_helper_with_term(rego.metadata.chain(), [name], name),
		inherited_label,
	)
}
...
deny contains result if {
	...
	result := utils.result_helper(rego.metadata.chain(), [_parent.ref, input.image.ref])
}
...
required_labels := utils.rule_data("required_labels") if {
	not is_fbc
} else := utils.rule_data("fbc_required_labels")

optional_labels := utils.rule_data("optional_labels") if {
	not is_fbc
} else := utils.rule_data("fbc_optional_labels")

disallowed_inherited_labels := utils.rule_data("disallowed_inherited_labels") if {
	not is_fbc
} else := utils.rule_data("fbc_disallowed_inherited_labels")
