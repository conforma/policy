...
test_expected_predicate_type if {
	...
	utils.assert_empty(slsa_provenance_available.deny) with input.attestations as attestations
}
...
test_att_predicate_type if {
	...
	utils.assert_equal_results(slsa_provenance_available.deny, expected_deny) with input.attestations as attestations
}
...
test_rule_data_format if {
	...
	utils.assert_equal_results(slsa_provenance_available.deny, expected) with data.rule_data as d
